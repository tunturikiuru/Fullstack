...
describe('total likes', () => {
    ...
    test('of a bidder list is calculated right', () => {
...
describe('most blogs', () => {
    test('of empty list is is null author with 0 blogs', () => {
        const result = listHelper.mostBlogs(blogsEmpty)
        assert.deepStrictEqual(result, {author: null, blogs: 0})
    })
    test('of list of one blog is author of that one with 1 blog', () => {
        const result = listHelper.mostBlogs([blogs[0]])
        assert.deepStrictEqual(result, {author: blogs[0].author, blogs: 1})
    })
    test('of bigger list is right author with correct blog amount', () => {
...
describe('most likes', () => {
    test('of empty list is null author with 0 likes', () => {
        const result = listHelper.mostLikes(blogsEmpty)
        assert.deepStrictEqual(result, {author: null, likes: 0})
    })
    test("of list of one blog is author of that one with blog's like count", () => {
        const result= listHelper.mostLikes([blogs[0]])
        assert.deepStrictEqual(result, {author: blogs[0].author, likes: blogs[0].likes})
    })
    test('of bigger list is right author with correct likes amount', () => {
